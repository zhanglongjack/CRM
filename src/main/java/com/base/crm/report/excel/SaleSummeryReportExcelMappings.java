...
public class SaleSummeryReportExcelMappings extends ExcelMappingsAbstract {
	...
	private static List<String> headerList = Arrays.asList( 
			"月份","采购成本","广告消耗","实际广告费","实发薪资",
			"快递费用","快递佣金","拒签已收额","未签总额",
			"预计业绩","实收总额","预计利润","实际利润");
	
	public static List<String> columnsMappings = Arrays.asList(
			"month","procurementCosts","consumeAD","realConsumeAD","realSalary",
			"expressTotalFee","expressCommissionTotalFee","realAmount","refuseAmount",
			"expectAmount","refusePaymentAmount","profit","realProfit"
			);
...
//	private ServerSalaryService serverSalaryService;
	...
	public ExcelMappingsAbstract statistics(SummaryReport summaryReport) {
		...
		orderParams.setStartDate(summaryReport.getStartDate());
		orderParams.setEndDate(summaryReport.getEndDate());
		reportResult = custOrderService.selectSalesPerformanceSummaryReport(orderParams);
		
		SummaryReport sumReport = new SummaryReport();
		sumReport.setDates("总计");
		sumReport.setMonth("总计");
		for(SummaryReport report : reportResult){
			//预计利润 = =预计业绩总额 - 实际广告费 - 采购成本 - 实发薪资 - 快递费用 - 快递佣金
